...
const CourseContentEditor = () => {
  ...
  useEffect(() => {
    const fetchCourseData = async () => {
      try {
        ...
        // Estructura segura para el contenido
        ...
        showToast('error', 'Error', 'No se pudo cargar el contenido del curso.');
      } finally {
        setLoading(false);
      }
    };
        ...
        <div style={{ paddingLeft: '1rem', borderLeft: '2px solid #eee' }}>
          {module.lessons && module.lessons.map(lesson => (
            ...
            >
              <div>
                <i 
                  className={`pi ${lesson.type === 'video' ? 'pi-youtube' : 'pi-file'}`} 
  ...
  return (
    <div style={{ padding: '2rem' }}>
        ...
        <Card title={
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <p className="p-text-secondary">
              Organiza los módulos de tu curso. Puedes arrastrarlos para cambiar el orden.
            </p>
            <Button 
              label="Añadir Módulo" 
              icon="pi pi-plus" 
      ...
      {/* Modal para Módulos */}
      <Dialog 
        header={currentModule ? 'Editar Módulo' : 'Nuevo Módulo'} 
        visible={moduleModalVisible} 
        onHide={hideModuleModal} 
        style={{ width: '30rem' }} 
        modal
      >
      ...
      <Dialog 
        header={currentLesson ? 'Editar Lección' : 'Nueva Lección'} 
        visible={lessonModalVisible} 
        onHide={hideLessonModal} 
        style={{ width: '40rem' }} 
        modal
      >
        <div className="p-fluid">
          ...
          <div className="p-field" style={{ marginBottom: '1.5rem' }}>
            ...
            <Dropdown 
              ...
              options={lessonTypes} 
              onChange={(e) => setLessonData({...lessonData, type: e.value})} 
              optionLabel="label"
            />
          </div>
          ...
          {lessonData.type === 'text' && (
            <div className="p-field" style={{ marginBottom: '1.5rem' }}>
              ...
              <Editor 
                ...
                onTextChange={(e) => setLessonData({...lessonData, textContent: e.htmlValue})} 
              />
            </div>
          )}
          <div style={{ marginTop: '1.5rem', textAlign: 'right' }}>
